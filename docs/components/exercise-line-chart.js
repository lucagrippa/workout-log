...
export function calculateMaxWeightPerExercisePerDay(workouts) {
    ...
    for (const date in workouts) {
        ...
        for (const exercise in maxWeightPerExercise) {
            maxWeightPerExercisePerDay.push({ date, exercise, weight: maxWeightPerExercise[exercise] });
        }
...
export function exerciseLineChart(workouts, exercise, color, { width }) {
    ...
    const xMinDomain = d3.utcDay.offset(d3.min(maxWeightPerExercisePerDay, d => d.date), -2);
    const xMaxDomain = d3.utcDay.offset(d3.max(maxWeightPerExercisePerDay, d => d.date), 1);
